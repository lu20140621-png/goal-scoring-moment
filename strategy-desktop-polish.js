  ...
  function applyDesktopPolish(){
    ...
    st.textContent=`
      @media (min-width:761px){
        ...
        .gsmSoccerFlow{display:flex;align-items:center;gap:7px;flex:0 0 auto}.gsmSoccerCardImage{width:82px;height:82px;object-fit:contain;display:block;filter:drop-shadow(0 5px 8px #0007)}.gsmSoccerText{max-width:125px;font-size:9px;line-height:1.25;font-weight:900;color:#e4eef8}
      }
    `;
    ...
    function upgradeSoccerResults(){
      ...
      [...played.querySelectorAll('.resultChip')].forEach(chip=>{
        ...
        wrap.innerHTML=`<img class="gsmSoccerCardImage" src="images/soccer-card.png" alt="Soccer Card"><div class="gsmSoccerText">${txt}</div>`;
    ...
    const lesson=d.querySelector('.lesson');
    if(lesson){
      new w.MutationObserver(polishPassLesson).observe(lesson,{childList:true,subtree:true,characterData:true});
    }
