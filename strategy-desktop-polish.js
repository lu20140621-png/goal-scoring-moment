  ...
  function applyDesktopPolish(){
    ...
    st.textContent=`
      @media (min-width:761px){
        ...
        .lesson.gsmPassLesson .flow{margin-top:7px;line-height:1.25;white-space:normal}
    ...
    function polishPassLesson(){
      ...
      if(isPass){
        const wanted='Pass the Soccer Card to BLUE 2.<br>Possession changes, but your turn does not end.';
        if(hint.innerHTML!==wanted)hint.innerHTML=wanted;
      }
    }
    ...
    function ensureStartingSoccerCard(){
      ...
      if(!played||played.querySelector('.gsmSoccerStart,.gsmPlayedSoccerStart'))return;
