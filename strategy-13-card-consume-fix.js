...
/* Lessons 1–3 physical-card onboarding.
   Coach asks for both teams' role cards, but the visual tray only shows the four cards the learner follows directly. */
(() => {
  'use strict';
  if (window.__gsmStrategyPhysicalPrepInstalled) return;
  window.__gsmStrategyPhysicalPrepInstalled = true;

  const PREP_TEXT = 'Before we start, take out the BLUE Team GOALKEEPER and PLAYER, the GREEN Team GOALKEEPER and PLAYER, 1 SOCCER card, and any 1 ACTION card. Put the GREEN pair on the opposite side. Keep the BLUE Goalkeeper, BLUE Player, SOCCER, and ACTION card closest to you — these are the four cards you’ll follow with me.';
  let prepShown = false;
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .physicalPrepCard b{display:block;color:#183c60;font-size:7px;line-height:1.15;font-weight:950;overflow-wrap:anywhere}
      ...
      @media(max-width:760px){
        ...
        .physicalPrepCards{grid-template-columns:repeat(4,minmax(0,1fr));gap:4px}
      ...
      @media(max-width:350px){
        ...
        .physicalPrepCards{grid-template-columns:repeat(2,minmax(0,1fr))}
        .physicalPrepCard img{width:38px;height:52px}
  ...
  function showTray(dialogue) {
    ...
    tray.setAttribute('aria-label','Four cards to follow during the guided demo');
    tray.innerHTML = `
      ...
      <div class="physicalPrepHint">Your GREEN Goalkeeper and Player stay on the opposite side. These four are the cards you’ll move with the coach.</div>`;
